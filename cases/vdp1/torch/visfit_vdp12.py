...
try:
    historicdf = pd.read_csv('historic_torch_vdp1.csv')
    historicdf.loc[len(historicdf)] = logdf.iloc[0]
    historicdf.to_csv('historic_torch_vdp1.csv', index=False)
except:
    try:
        logdf.to_csv('historic_torch_vdp1.csv', index=False)
    except:
        print('Could not save historic_torch_vdp1')

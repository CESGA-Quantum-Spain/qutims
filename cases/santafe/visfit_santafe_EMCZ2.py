...
#fname_p = 'param0_'+job_id+'.dat' #PARAMETERS
...
try:
    historicdf = pd.read_csv('historic.csv')
    historicdf.loc[len(historicdf)] = logdf.iloc[0]
    historicdf.to_csv('historic.csv', index=False)
except:
    try:
        logdf.to_csv('historic.csv', index=False)
    except:
        print('Could not save historic')

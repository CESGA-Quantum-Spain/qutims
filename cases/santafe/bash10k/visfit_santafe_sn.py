...
shots = False # !!! ativate or deactivate shots. If activated, the number is the same one as in training.
if shots:
    nshots = int(log_data['Nshots'])
    print('Using nshots=%i set in training' %nshots)
else:
    nshots = 0
    print('Using nshots=0, instead of %i set in training' %int(log_data['Nshots']))
...
try:
    historicdf = pd.read_csv('historic_emcz2_santafe_sn%i.csv' %int(logdf['Nshots']))
    historicdf.loc[len(historicdf)] = logdf.iloc[0]
    historicdf.to_csv('historic_emcz2_santafe_sn%i.csv' %int(logdf['Nshots']), index=False)
except:
    try:
        logdf.to_csv('historic_emcz2_santafe_sn%i.csv' %int(logdf['Nshots']), index=False)
    except:
        print('Could not save historic_emcz2_santafe_sn%i' %int(logdf['Nshots']))

...
train_outputs = model(train_sequences).detach().numpy()
val_outputs   = model(val_sequences).detach().numpy()
test_outputs  = model(test_sequences).detach().numpy()
filltest_outputs = model(filltest_sequences).detach().numpy()
...
try:
    ...
    historicdf.to_csv('historic_torch_dectriang.csv', index=False)
except:
    try:
        logdf.to_csv('historic_torch_dectriang.csv', index=False)
    except:
        print('Could not save historic_torch_dectriang')

...
sequences = data[:,1:3].reshape(nsamples,n,2)
tarseq = data[:,3].reshape(nsamples,n)
targets =   np.array([item[-N:] for item in data[:,3].reshape(nsamples,n)])
...
filltest_sequences = torch.tensor(np.array([data[:,1:3][trval_npoints-n+N+N*i:trval_npoints-n+N+N*i+n] for i in range(ts_npoints//N)])).float()#.unsqueeze(-1)
filltest_targets = torch.tensor(np.array([data[:,3][trval_npoints+N*i:trval_npoints+N*i+N] for i in range(ts_npoints//N)])).float()
...
try:
    historicdf = pd.read_csv('historic_torch_vdp2.csv')
    historicdf.loc[len(historicdf)] = logdf.iloc[0]
    historicdf.to_csv('historic_torch_vdp2.csv', index=False)
except:
    try:
        logdf.to_csv('historic_torch_vdp2.csv', index=False)
    except:
        print('Could not save historic_torch_vdp2')

...
if shots:
    ...
    nshots = 0
    print('Using nshots=0, instead of %i set in training' %nshots)
...
try:
    historicdf = pd.read_csv('historic_emcz2_santafe_sn%i.csv' %logdf['nshots'])
    historicdf.loc[len(historicdf)] = logdf.iloc[0]
    historicdf.to_csv('historic_emcz2_santafe_sn%i.csv' %logdf['nshots'], index=False)
except:
    try:
        logdf.to_csv('historic_emcz2_santafe_sn%i.csv' %logdf['nshots'], index=False)
    except:
        print('Could not save historic_emcz2_santafe_sn%i' %logdf['nshots'])

...
Description: this script creates the class and runs LSTMModel training, running the samples (windows of the series) with HAND-MADE lines of code.
...
#### DATA PREPARATION  ##################################################################
fname = 'data_vdp_mu_1_3_del_5_16_ab_100_1000p.dat'
data = np.loadtxt('../'+fname)

#x_data = np.array([[data[i,1]] for i in range(len(data[:,1]))]) # 2D-array!
x_data = np.array([[data[i,1],data[i,2]] for i in range(len(data[:,1]))]) # 2D-array!
...
ddist_seed = 0 # data distribution seed
...
Sinput = 2; Shidden = 2; Sout = 1; Nlayers = 1

model = LSTMModel(input_size=Sinput, hidden_size=Shidden, output_size=Sout, num_layers=Nlayers)
